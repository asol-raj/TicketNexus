...
async function initManagerAwareEmployeeForm() {
  ...
  // const hiddenManager = document.getElementById("singleManagerId");

  try {
    const data = await getJSON("/internal/data/managers");
    const managers = data.managers || []; console.log(managers);
    ...
    if (managers.length === 1) {
      ...
      // hiddenManager.value = managers.manager_employee_id
...
function bindCreateForms() {
  ...
  bind("createEmployeeForm", "employeeMsg", "/internal/employees", (payload) => { //console.log(payload)
