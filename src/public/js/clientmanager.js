...
async function refreshTickets(filter="all"){
  try{const d=await getJSON("/client-manager/data/tickets");
    let rows=d.tickets||[];
    ...
    renderTicketsTable(rows); renderRecentList(d.tickets||[]);
  }catch{}
}
...
async function populateAssignees(){
  ...
  try{const d=await getJSON("/client-manager/data/assignees");
