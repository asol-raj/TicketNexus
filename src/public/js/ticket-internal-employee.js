// src/public/js/ticket-internal-employee.js
        ...
        try {
          ...
          <div class="border rounded p-2 h-100">
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <button class="btn btn-sm btn-outline-danger delete-attachment" data-id="${a.id}"><i class="bi bi-trash"></i></button>
            </div>
            ${isImg
    ...
    q(".self-assign").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (j.success) {
                    // Remove button and mark as taken without reload
                    btn.closest(".list-group-item").innerHTML =
                        `<div><strong>#${id}</strong> · Assigned to you</div>`;
                } else {
        ...
        // ----- Update status -----
    q("#updateStatusBtn")?.addEventListener("click", async () => {
      const status = q("#statusSelect").value;
      try {
        const res = await fetch(`/employee/tickets/${TICKET_ID}/status`, {
          method: "POST",
